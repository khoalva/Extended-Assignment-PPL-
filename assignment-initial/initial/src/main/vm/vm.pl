...
update_env(Var, Value, env([Scope|Rest], Status), env([NewScope|Rest], Status)) :-
    has_declared(Var, Scope, id(Var, var, Type, _)), !, % Check if Var exists
...
replace_var([id(Var, var, _, _)|T], Var, Value, [id(Var, var, _, Value)|T]) :- !.
...
reduce_all(config(V, Env), config(Value, Env)) :- 
    atom(V), 
    has_declared(V, Env, id(V, var, _, Value)), !.
reduce_all(config(V, Env), config(V, Env)) :- 
    integer(V), !.
